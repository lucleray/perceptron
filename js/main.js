...
function traceline(wx,wy,t) {
	...
	if(wy != 0) {
		...
		if(y0 >= 0 && y0 <= 400) { plinearr.push({x:0,y:y0}); }
		if(y400 >= 0 && y400 <= 400) { plinearr.push({x:400,y:y400}); }
	}
	if(wx != 0) {
		var x0 = (t*400)/wx;
		var x400 = (t*400)/wx-(wy/wx)*400;
		if(x0 >= 0 && x0 <= 400) { plinearr.push({x:x0,y:0}); }
		if(x400 >= 0 && x400 <= 400) { plinearr.push({x:x400,y:400}); }
	}
	
	if(plinearr.length >= 3 && plinearr[0].y == plinearr[1].y && plinearr[0].x == plinearr[1].x) 
	...
	else { 	$('horsecran').setStyle('display','block'); }
}
...
function entrainep(echantillon)
{
	...
	if($('corepx').get('value') == wx && $('corepy').get('value') == wy && $('corept').get('value') == t)
	{ var fin = true; }
	else { var fin = false; }
	$('corepx').set('value',wx);
	$('corepy').set('value',wy);
...
//ajout de point
	...
	$('graph').addEvent('click',function(e){
		...
		//on rajoute le point dans la liste
			...
			nplist.set('html',
				'<div class="ppoint" style="background:'+colorp+'"></div>'
				+'<div>'+idp+'</div>'
				+'<div><span>'+pmx/400+'</span></div>'
				+'<div><span>'+pmy/400+'</span></div>'
				+'<hr />'
			);
			$('ulist').grab(nplist);

		//on rajoute la suppression possible du point créé
		...
		//on rajoute le déplacement du point
...
//on change le perceptron dès qu'un poids est changé
...
//tracé au début
traceline($('corepx').get('value'),$('corepy').get('value'),$('corept').get('value'));


//fonction pour ajouter une aide sur un élément
...
aide('graph','Cliquez pour ajouter/déplacer/supprimer un point (choisissez le mode à l\'aide des liens sous le graphe)');
aide('addp','Lorsque ce mode est sélectionné, vous ajoutez des points sur le graphe');
aide('movep','Lorsque ce mode est sélectionné, vous déplacez les points sur le graphe');
aide('deletep','Lorsque ce mode est sélectionné, vous supprimez les points sur le graphe');
...
aide('alphadiv','Cliquez pour modifier le coefficient d\'apprentissage du perceptron');
aide('csimple','Effectuez une seule étape d\'apprentissage');
aide('ctotal','Effectuez l\'apprentissage jusqu\'à ce que tout les points soient correctement classés');
